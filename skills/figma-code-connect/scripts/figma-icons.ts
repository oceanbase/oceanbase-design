/**
 * Sync Figma icons → `Icons/index.figma.tsx`, optional SVG under `packages/icons/svg`, then `pnpm --filter @oceanbase/icons generate`.
 *
...
function writeGeneratedFiles(rows) {
  ...
  fs.writeFileSync(
    iconsFigmaPath,
    `// @ts-nocheck\n/* eslint-disable */\n/* Auto-generated by skills/figma-code-connect/scripts/figma-icons.ts — no Icon* prefix; names align with packages/icons/svg/{theme}/{stem}.svg + theme suffix (Outlined|Filled|TwoTone|Colored) */\nimport { figma } from "@figma/code-connect";\n${importBlock}\n\n${connects}\n`
  );
  console.log(
...
function writeRawSvgFiles(
  ...
  options: { clean?: boolean; skipExisting?: boolean } = {}
) {
...
async function fileRESTResponseToIconRows(response) {
  ...
  if (stillFails.length > 0) {
    console.warn(
      `${stillFails.length} icon(s) still missing SVG after retries — Icons.figma.tsx will omit them. Re-run the script or raise FIGMA_IMAGE_WAIT_MS / FIGMA_SVG_FETCH_TIMEOUT_MS in figma-icons.ts.`
    );
  }
