 ...
 * 文档为交互演示：`open` 默认关闭、`getContainer` 限制在左侧预览区、按钮打开；Code Connect 侧为 `open={true}` 全页语义。
...
const App: React.FC = () => {
  ...
  const { width, closable, title, footer, modalBody } = useMemo(() => {
    const w = type === 'confirm' ? 384 : 520;
    const c = type !== 'confirm';
    const t = type === 'confirm' ? null : titleText;
