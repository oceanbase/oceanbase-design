import { Col, Row, Select, Space, Steps, Typography } from '@oceanbase/design';
import React, { useState } from 'react';

/**
 * Playground：与 `../index.figma.tsx` 中 `figma.connect(Steps.Step, '<FIGMA_OCEANBASE_STEPITEM>')` 的
 * `props`（`status`）与 `example` 拼装一致。Figma `current` → antd `process`。
 */

type StepStatusCode = 'wait' | 'finish' | 'process';

/** 与 figma.connect example 相同的拼装方式 */
function FigmaStepItemExample(props: { status: StepStatusCode }) {
  const { status } = props;
  return (
    <Steps current={0}>
...
const App: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        <FigmaStepItemExample status={status} />
      </Col>
      <Col
        flex="0 0 280px"
        style={{
