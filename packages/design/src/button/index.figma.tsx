...
/**
 * Icons：`PlusOutlined` / `DownOutlined` / `EllipsisOutlined` — Figma「icontype」与稿面操作图标语义；
 * `@oceanbase/icons` 无等价细粒度 outlined 变体，沿用 `@ant-design/icons`。
 */
...
figma.connect(Space, '<FIGMA_OCEANBASE_BUTTON_GROUP>', {
  props: {
    ...
    // righttoleft 由子节点顺序表达；`direction: rtl` 仅在 demo/button-group.figma.tsx 预览容器
    children: figma.enum('type', {
  ...
  example: ({ size, children }) => <Space size={size}>{children}</Space>,
});
