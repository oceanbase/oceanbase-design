...
/**
 * Playground：`../index.figma.tsx` 的 `example` 仅为 `<Card bordered={false}>{children}</Card>`；
 * 卡片尺寸、阴影、body 内边距等预览样式在本文件外层 `Card` 上维护（§3.4a）。
...
/** 与映射文件目标一致：`Card` + `children`；稿面像素仅在此处注入（§3.4a）。 */
function FigmaNotificationExample(props: { children: React.ReactNode }) {
  const { children } = props;
  return (
    <Card style={CARD_STYLE} styles={CARD_STYLES} bordered={false}>
      {children}
    </Card>
  );
}
...
const App: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        <FigmaNotificationExample>{children}</FigmaNotificationExample>
      </Col>
      <Col
        flex="0 0 280px"
        style={{
