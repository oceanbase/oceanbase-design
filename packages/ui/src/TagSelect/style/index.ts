...
const genColoredStyle = (borderColor: string, backgroundColor: string, color: string): CSSObject => {
    return {
        borderColor,
        backgroundColor,
        color
...
export const genTagSelectStyle: GenerateStyle<TagSelectToken> = (token: TagSelectToken): CSSObject => {
    const {
        componentCls,
    } = token;
