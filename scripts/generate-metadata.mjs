...
copyFileSync(
  join(root, 'metadata/obToken.css-vars.json'),
  join(srcDir, 'obToken.css-vars.json'),
);
copyFileSync(
  join(root, 'metadata/css-token-migration-hints.yaml'),
  join(srcDir, 'css-token-migration-hints.yaml'),
);

console.log(`generate:metadata wrote ${Object.keys(components).length} components + extracted props + css vars`);
