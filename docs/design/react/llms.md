...
| <a href="/design.md" target="_blank" rel="noopener noreferrer">design.md</a> | 设计语言（antd 基线 + OB 增量） |
...
> **Agent 抓取**：将路径拼到当前站点 origin（本地一般为 `http://localhost:8000`，生产为 `https://design.oceanbase.com`）。

## 在 AI 工具中使用

### Cursor

- 使用 **@Docs** 添加站点的 `/llms.txt`（本地示例：`http://localhost:8000/llms.txt`），或
- 在 `.cursor/rules` 中加入：

```text
编写 OceanBase Design 代码前，阅读 /docs/react/for-agents.md 与 /llms-ob-constraints.txt（前缀当前站点 origin）。组件 API 以 ob-design mcp 的 ob_info 为准，禁止 from 'antd'。
```
...
```bash
npx @oceanbase/design-cli setup --client claude
```
...
将以下路径交给 Agent 阅读（按优先级，需拼接站点 origin）：

1. `/docs/react/for-agents.md`
2. `/llms-ob-constraints.txt`
3. `/llms-full.txt`（上下文充足时）
